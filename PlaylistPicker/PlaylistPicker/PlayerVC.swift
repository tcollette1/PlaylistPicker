...
class PlayerVC: UIViewController {
    ...
    func fetchTheBlues() {
        ...
        queueArrays.myQuery.addFilterPredicate(voicerPredicate)
        var voiceCollection = [MPMediaItem]()
        let unixDate = Date(timeIntervalSinceNow: -1250000)
        if let voicers = queueArrays.myQuery.items {
            voiceCollection = voicers.sorted{$0.lastPlayedDate ?? unixDate < $1.lastPlayedDate ?? unixDate}
        }
        queueArrays.myQuery.removeFilterPredicate(voicerPredicate)
    ...
    func fetchRockPlaylist() {
        idArtist = myLists[0].idArtist!
        let unixDate = Date(timeIntervalSinceReferenceDate: 0)
        let thisTime = Double(Int(myLists[0].lastTime)*(-3600))
        let lastTime = Date(timeIntervalSinceNow:thisTime) // -129600 = 36 hours
        let rockPredicate6 = MPMediaPropertyPredicate(value: "60s Rock",
                                                 forProperty: MPMediaPlaylistPropertyName,
                                                 comparisonType: .equalTo)
        let rockPredicate7 = MPMediaPropertyPredicate(value: "70s Rock",
                                                 forProperty: MPMediaPlaylistPropertyName,
                                                 comparisonType: .equalTo)
        let rockPredicate8 = MPMediaPropertyPredicate(value: "80s Rock",
                                                 forProperty: MPMediaPlaylistPropertyName,
                                                 comparisonType: .equalTo)
        let rockPredicate9 = MPMediaPropertyPredicate(value: "90s Rock",
                                                 forProperty: MPMediaPlaylistPropertyName,
                                                 comparisonType: .equalTo)
        queueArrays.myQuery.addFilterPredicate(rockPredicate6)
        var collection6 = [MPMediaItem]()
        if var songs6 = queueArrays.myQuery.items {
            songs6 = songs6.filter { ($0.lastPlayedDate ?? unixDate) < lastTime as Date }
            if mp.playbackState != .stopped {
                collection6 = songs6.filter{($0.persistentID != songData.pID)}
            }
            else {
                collection6 = songs6
            }
        }
        queueArrays.myQuery.removeFilterPredicate(rockPredicate6)
        collection6 = collection6.shuffled()
        queueArrays.myQuery.addFilterPredicate(rockPredicate7)
        var collection7 = [MPMediaItem]()
        if var songs7 = queueArrays.myQuery.items {
           songs7 = songs7.filter { ($0.lastPlayedDate ?? unixDate) < lastTime as Date }
            if mp.playbackState != .stopped {
                collection7 = songs7.filter{($0.persistentID != songData.pID)}
            }
            else {
                collection7 = songs7
            }
        }
        queueArrays.myQuery.removeFilterPredicate(rockPredicate7)
        collection7 = collection7.shuffled()
        queueArrays.myQuery.addFilterPredicate(rockPredicate8)
        var collection8 = [MPMediaItem]()
        if var songs8 = queueArrays.myQuery.items {
            songs8 = songs8.filter { ($0.lastPlayedDate ?? unixDate) < lastTime as Date }
            if mp.playbackState != .stopped {
                collection8 = songs8.filter{($0.persistentID != songData.pID)}
            }
            else {
                collection8 = songs8
            }
        }
        queueArrays.myQuery.removeFilterPredicate(rockPredicate8)
        collection8 = collection8.shuffled()
        queueArrays.myQuery.addFilterPredicate(rockPredicate9)
        var collection9 = [MPMediaItem]()
        if var songs9 = queueArrays.myQuery.items {
            songs9 = songs9.filter { ($0.lastPlayedDate ?? unixDate) < lastTime as Date }
            if mp.nowPlayingItem != nil {
                collection9 = songs9.filter{($0.persistentID != songData.pID)}
            }
            else {
                collection9 = songs9
            }
        }
        queueArrays.myQuery.removeFilterPredicate(rockPredicate9)
        collection9 = collection9.shuffled()
        var interDecade = 0 // step by 1 for position in each decade
        //var interDecade7 = 0 // separate counter for 70s decade
        var artistsList = artists[0..<Int(myLists[0].sepNum)]
        let voicerPredicate = MPMediaPropertyPredicate(value: "\(myLists[0].idList!)",
                                                       forProperty: MPMediaPlaylistPropertyName,
                                                       comparisonType: .equalTo)
        ...
        queueArrays.myList = [MPMediaItem]()
        repeat {
            for performer6 in collection6 {
                let artist6 = performer6.artist ?? "No Artist"
                if artistsList.contains(where: artist6.contains) {
                    if let index = collection6.firstIndex(of: performer6) {
                        collection6.remove(at: index)
                    }
                }
                else {
                    artistsList.removeLast()
                    artistsList.insert(artist6, at: 0)
                    queueArrays.myList.append(performer6)
                    break
                }
            }
            for performer8 in collection8 {
                let artist8 = performer8.artist ?? "No Artist"
                if artistsList.contains(where: artist8.contains) {
                    if let index = collection8.firstIndex(of: performer8) {
                        collection8.remove(at: index)
                    }
                }
                else {
                    artistsList.removeLast()
                    artistsList.insert(artist8, at: 0)
                    queueArrays.myList.append(performer8)
                    break
                }
            }
            for performer7 in collection7 {
                let artist7 = performer7.artist ?? "No Artist"
                if artistsList.contains(where: artist7.contains) {
                    if let index = collection7.firstIndex(of: performer7) {
                        collection7.remove(at: index)
                    }
                }
                else {
                    artistsList.removeLast()
                    artistsList.insert(artist7, at: 0)
                    queueArrays.myList.append(performer7)
                    break
                }
            }
            for performer9 in collection9 {
                let artist9 = performer9.artist ?? "No Artist"
                if artistsList.contains(where: artist9.contains) {
                    if let index = collection9.firstIndex(of: performer9) {
                        collection9.remove(at: index)
                    }
                }
                else {
                    artistsList.removeLast()
                    artistsList.insert(artist9, at: 0)
                    queueArrays.myList.append(performer9)
                    break
                }
            }
           for performer70 in collection7 {
               let artist70 = performer70.artist ?? "No Artist"
               if artistsList.contains(where: artist70.contains) {
                   if let index = collection7.firstIndex(of: performer70) {
                       collection7.remove(at: index)
                   }
               }
               else {
                   artistsList.removeLast()
                   artistsList.insert(artist70, at: 0)
                   queueArrays.myList.append(performer70)
                   break
               }
           }
            interDecade += 1
        } while interDecade < voiceCollection.count
        var interR = 0
        var indexV = 0
        if myLists[0].idFreq < 10 {
            if voiceCollection.count * Int(myLists[0].idFreq) > queueArrays.myList.count {
